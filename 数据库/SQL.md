...
**多表查询**：从多张表中查询数据。其数学基础是**笛卡尔积**——两个集合所有元素的**全部组合**（如 2 行 × 4 行 = 8 行）。不加连接条件就会得到这些无意义的组合，所以**连接条件的作用就是从笛卡尔积中筛出有意义的行**。

各类连接按**集合语义**区分：

| 连接 | 取的是 | 说明 |
| --- | --- | --- |
| **内连接** | 交集 A∩B | 两表中**同时匹配**的数据 |
| **左外连接** | 整张 A + B 中匹配部分 | 保留左表全部行，右表无匹配时字段为 `NULL` |
| **右外连接** | 整张 B + A 中匹配部分 | 与左外连接方向相反 |

```sql
-- 内连接：隐式（常见）与显式两种写法，效果相同
...
> **右外连接总能通过交换两表顺序改写成左外连接**，所以实际开发中常用左外连接即可。
> 连接条件（`ON` / `WHERE`）在第 1 步 `FROM / JOIN` 就生效（见第 3 节执行顺序），早于过滤条件。

## 5. 子查询

SQL 语句中**嵌套 SELECT**，又称嵌套查询。外部语句可以是 `insert` / `update` / `delete` / `select` 任何一个，最常见的是 `select`。
...
| --- | --- | --- |
| **标量子查询** | 单个值 | 用 `=`、`>` 等比较 |
| **列子查询** | 一列 | 配合 `in` / `not in` |
| **行子查询** | 一行 | 配合 `(列1, 列2) = (...)` 整行比较 |
| **表子查询** | 多行多列 | 当作**派生表**放进 `from` |

> 子查询的要点：**先对需求做拆分、明确具体步骤，再逐步编写 SQL**

```sql
-- 表子查询:获取每个部门中薪资最高的员工信息
